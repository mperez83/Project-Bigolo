...
void UMainCharacterMovementComponent::SetSprinting(bool sprinting)
{
	SprintKeyDown = sprinting;
}

bool UMainCharacterMovementComponent::BeginWallRun()
{
...
void UMainCharacterMovementComponent::EndWallRun()
{
	// Set the movement mode back to falling
	SetMovementMode(EMovementMode::MOVE_Falling);
}

bool UMainCharacterMovementComponent::AreRequiredWallRunKeysDown() const
{
...
bool UMainCharacterMovementComponent::IsNextToWall(float vertical_tolerance)
{
...
void UMainCharacterMovementComponent::FindWallRunDirectionAndSide(const FVector& surface_normal, FVector& direction, EWallRunSide& side) const
{
	FVector crossVector;
...
bool UMainCharacterMovementComponent::CanSurfaceBeWallRan(const FVector& surface_normal) const
{
...
bool UMainCharacterMovementComponent::IsCustomMovementMode(uint8 custom_movement_mode) const
{
	return MovementMode == EMovementMode::MOVE_Custom && CustomMovementMode == custom_movement_mode;
}

void UMainCharacterMovementComponent::OnActorHit(AActor* SelfActor, AActor* OtherActor, FVector NormalImpulse, const FHitResult& Hit)
{
...
void UMainCharacterMovementComponent::BeginPlay()
{
	...
	if (GetPawnOwner()->GetLocalRole() > ROLE_SimulatedProxy)
	{
		...
		GetPawnOwner()->OnActorHit.AddDynamic(this, &UMainCharacterMovementComponent::OnActorHit);
	}
}

void UMainCharacterMovementComponent::OnComponentDestroyed(bool bDestroyingHierarchy)
{
	if (GetPawnOwner() != nullptr && GetPawnOwner()->GetLocalRole() > ROLE_SimulatedProxy)
	{
		// Unbind from all events
		GetPawnOwner()->OnActorHit.RemoveDynamic(this, &UMainCharacterMovementComponent::OnActorHit);
	}
...
void UMainCharacterMovementComponent::PhysWallRunning(float deltaTime, int32 Iterations)
{
...
void UMainCharacterMovementComponent::ProcessLanded(const FHitResult& Hit, float remainingTime, int32 Iterations)
{
...
bool UMainCharacterMovementComponent::DoJump(bool bReplayingMoves)
{
	if (CharacterOwner && CharacterOwner->CanJump())
	{
		...
		if (!bConstrainToPlane || FMath::Abs(PlaneConstraintNormal.Z) != 1.f)
		{
			...
			velNormalized.Normalize(0.0001f);

			Velocity.X += (velNormalized.X * JumpForwardAddVelocity);
			Velocity.Y += (velNormalized.Y * JumpForwardAddVelocity);
...
uint8 FSavedMove_Main::GetCompressedFlags() const
{
...
bool FSavedMove_Main::CanCombineWith(const FSavedMovePtr& NewMovePtr, ACharacter* Character, float MaxDelta) const
{
	const FSavedMove_Main* NewMove = static_cast<const FSavedMove_Main*>(NewMovePtr.Get());
...
void FSavedMove_Main::SetMoveFor(ACharacter* Character, float InDeltaTime, FVector const& NewAccel, class FNetworkPredictionData_Client_Character& ClientData)
{
	Super::SetMoveFor(Character, InDeltaTime, NewAccel, ClientData);

	UMainCharacterMovementComponent* charMov = static_cast<UMainCharacterMovementComponent*>(Character->GetCharacterMovement());
	if (charMov)
	{
...
void FSavedMove_Main::PrepMoveFor(class ACharacter* Character)
{
...
FNetworkPredictionData_Client_Main::FNetworkPredictionData_Client_Main(const UCharacterMovementComponent& ClientMovement)
	: Super(ClientMovement)
{

}

FSavedMovePtr FNetworkPredictionData_Client_Main::AllocateNewMove()
{
