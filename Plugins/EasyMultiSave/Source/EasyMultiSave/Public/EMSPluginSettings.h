...
public:
	...
	UPROPERTY(config, EditAnywhere, Category = "General Settings", meta = (MetaClass = "EMSPersistentSaveGame"))
	...
	UPROPERTY(config, EditAnywhere, Category = "General Settings", meta = (MetaClass = "EMSInfoSaveGame"))
	...
	UPROPERTY(config, EditAnywhere, Category = "General Settings", meta = (MetaClass = "EMSProfileSaveGame"))
	...
	/**If enabled, the name of the level a placed Actor belongs to is saved. Useful for Level Streaming. */
	UPROPERTY(config, EditAnywhere, Category = "Actors", meta = (DisplayName = "Save Actor Affiliation"))
	bool bSaveActorAffiliation = false;

	/**
	Struct and Map variables with 'Save Game' checked are fully saved automatically for all Actors. Otherwise only the desired variables inside a struct are saved.
	*/
	...
	EFileSaveMethod FileNamingType = EFileSaveMethod::FM_Optimized;
