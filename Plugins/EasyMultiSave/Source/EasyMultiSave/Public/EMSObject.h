...
public:
	...
	FORCEINLINE void ClearCachedPersistentAndSlot()
	{
	...
	FORCEINLINE FString CustomSaveFile(const FString& CustomSaveName, const bool bUseSlot = false)
	{
		if (bUseSlot)
		{
			const FString SaveSlotName = GetCurrentSaveGameName();
			return SaveFolder(SaveSlotName) + CustomSaveName + SaveType;
		}

		return  BaseSaveDir() + CustomSaveName + SaveType;
	}
	...
	FORCEINLINE FString GetFullActorName(AActor* Actor)
	{
		FString ActorName = Actor->GetName();

		if (UEMSPluginSettings::Get()->bSaveActorAffiliation)
		{
