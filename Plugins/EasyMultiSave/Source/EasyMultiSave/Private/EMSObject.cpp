...
This will replace Persistent and Profile in the Future.
**/
...
UEMSCustomSaveGame* UEMSObject::GetCustomSave(const TSubclassOf<UEMSCustomSaveGame>& SaveGameClass)
{
	...
	UEMSCustomSaveGame* CachedObject = CachedCustomSaves.FindRef(CustomSaveName);
	if (CachedObject)
	{
	...
	else
	{
		...
		CachedCustomSaves.Add(CustomSaveName, NewObject);
...
void UEMSObject::DeleteCustomSave(UEMSCustomSaveGame* SaveGame)
{
	...
	{
		bSuccess = IFileManager::Get().Delete(*SaveFile, true);
	}

	if (bSuccess)
	{
		const UEMSCustomSaveGame* CachedObject = CachedCustomSaves.FindRef(CustomSaveName);
		if (CachedObject)
		{
			CachedCustomSaves.Remove(CustomSaveName);
		}
...
void UEMSObject::DeleteLocalProfile()
{
	CachedProfileSave = nullptr;

	const FString SaveFile = ProfileSaveFile();
	bool bSuccess = false;

	bSuccess = IFileManager::Get().Delete(*SaveFile, true);
	if (bSuccess)
	{
		UE_LOG(LogEasyMultiSave, Log, TEXT("Local Profile Deleted"));
	}
}
	...
	for (AActor* Actor : ActorList)
	{
		if (Actor && IsValidForSaving(Actor))
		{
			...
			if (Type == EActorType::AT_Runtime || Type == EActorType::AT_Placed || Type == EActorType::AT_Persistent)
			{
				...
				//UE_LOG(LogEasyMultiSave, Warning, TEXT("Actor: %s"), *GetFullActorName(Actor));
...
bool UEMSObject::CheckForExistingActor(const FActorSaveData& ActorArray)
{
	...
	if (ThisWorld && ThisWorld->PersistentLevel)
	{
		...
		if (NewLevelActor)
		{
			if (!NewLevelActor->ActorHasTag(HasLoadedTag))
			{
...
TSaveGame* UEMSObject::GetDesiredSaveObject(const FString& FullSavePath, const FSoftClassPath& InClassName, TSaveGame*& SaveGameObject)
{
	...
	if (!SaveGameObject)
	{
		//Carefully get the class reference.
